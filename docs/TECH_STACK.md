...
6. ✅ **GUI** – Tauri 2 + SolidJS, reusing `manager-core` directly; no logic duplicated
   anywhere — sorting, the job engine, and the search/compare/duplicate walks are the
   same code the terminal calls, just through thin Tauri commands and events instead of
   a Ratatui event loop. Started as a deliberately thin shell (two panels, keyboard
   navigation, click/double-click, nothing else) and grew one slice at a time to the
   terminal version's full feature set: marking, copy/move/delete through the real job
